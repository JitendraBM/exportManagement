...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v39: quotations can be priced FOB-first - see the changelog entry
            # above. Existing quotations keep fob_pricing = 0 (prices typed are
            # already CIF, exactly as before) and their items keep a NULL
            # fob_price_usd, so nothing already saved reprices itself.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(quotations)")}
            if existing and "fob_pricing" not in existing:
                conn.execute("ALTER TABLE quotations ADD COLUMN fob_pricing INTEGER NOT NULL DEFAULT 0")
            if existing and "round_off" not in existing:
                conn.execute("ALTER TABLE quotations ADD COLUMN round_off REAL NOT NULL DEFAULT 0")
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(quotation_items)")}
            if existing and "fob_price_usd" not in existing:
                conn.execute("ALTER TABLE quotation_items ADD COLUMN fob_price_usd REAL")
