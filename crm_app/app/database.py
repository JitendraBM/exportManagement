...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v22: Buyer Order No & Date on an Export Invoice is a single
            # field (all PIs referenced by one EI share the same buyer
            # order), not a repeatable list - replaces the short-lived
            # export_invoice_buyer_orders table from v21 with two scalar
            # columns on export_invoices itself.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoices)")}
            if existing and "buyer_order_no" not in existing:
            ...
            # v22 (same pass): Booking No., printed above the 11B container
            # table on the Export Invoice's page-2 annexure.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoices)")}
            if existing and "booking_no" not in existing:
                conn.execute("ALTER TABLE export_invoices ADD COLUMN booking_no TEXT")

            # v23: a permit ("Permission", under Our Company) is no longer
            # tied to a supplier - it now records a stuffing-place name
            ...
            if existing:
                if "stuffing_place_name" not in existing:
                    conn.execute("ALTER TABLE permits ADD COLUMN stuffing_place_name TEXT")
                if "supplier_id" in existing:
                    conn.execute("ALTER TABLE permits DROP COLUMN supplier_id")
                # Self-heals a stray `stuffing_place_number` column some dev
                # databases picked up from an earlier draft of this migration
                # (the debug reloader re-runs init_schema on every file save,
                # so an in-progress edit here could already have executed).
                if "stuffing_place_number" in existing:
                    conn.execute("ALTER TABLE permits DROP COLUMN stuffing_place_number")
