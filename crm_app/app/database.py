...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v26: the Export Packing List. Its two tables are wholly new, so
            # schema.sql's CREATE TABLE IF NOT EXISTS covers them and nothing
            # is needed here - but the "Stuff At" address it prints is
            # captured on the parent export invoice, so that one column has
            # to be retrofitted onto existing export_invoices rows.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoices)")}
            ...
            # v27: the annexure's section-11B Tare Weight column used to be
            # printed blank for the examiner to fill in by hand; it is now
            # captured per container alongside the seal numbers.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoice_container_details)")}
            if existing and "tare_weight_kg" not in existing:
