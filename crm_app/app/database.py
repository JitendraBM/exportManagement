...
class Database:
    ...
    def _pre_schema_migrate(self) -> None:
        """Brings an existing DB to a shape schema.sql can run against, BEFORE
        it runs - which is what separates this from `_migrate`. schema.sql
        ends in CREATE INDEX statements, and an index naming a column that a
        CREATE TABLE IF NOT EXISTS could not retrofit would crash the whole
        script; so would an old-shape table surviving into it.

        Two things happen here: the v98 loading-planning reshape, whose new
        columns include an indexed one, and the old "start fresh" catalog and
        packing-list drops.
        """
        self._pre_schema_loading_planning()
        self._pre_schema_drop_legacy()

    def _pre_schema_loading_planning(self) -> None:
        """v98/v99: reshape the loading-planning tables ahead of schema.sql,
        which cannot retrofit a column onto an existing table and whose
        trailing CREATE INDEX statements would crash on a missing one.

        The DROPs here are real, which the migrations otherwise never do -
        safe only because v94 shipped this module before v96 existed and no
        loading planning has ever been saved against it, so there are no rows
        to carry forward and nothing to reconstruct them from. See the
        SCHEMA_VERSION changelog."""
        if not os.path.exists(self.db_path):
            return
        with self.get_connection() as conn:
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(loading_plannings)")}
            # v99 replaces v98's single packing_planning_id with a link table,
            # since one container load draws on several packing runs. Its
            # index has to go first - SQLite refuses to drop a column an
            # index still names.
            conn.execute("DROP INDEX IF EXISTS idx_loading_plannings_packing_plan")
            for column in ("packing_planning_id", "packing_planning_number"):
                if column in existing:
                    conn.execute(f"ALTER TABLE loading_plannings DROP COLUMN {column}")

            self._add_columns(conn, "loading_planning_items", {
                "batch_number": "TEXT",
                "production_date": "TEXT",
                "packing_planning_id": "INTEGER REFERENCES packing_plannings(id)",
                "packing_planning_number": "TEXT",
                "source_sr_no": "INTEGER",
            })
            self._add_columns(conn, "loading_planning_packings", {
                "packing_planning_id": "INTEGER REFERENCES packing_plannings(id)",
                "packing_planning_number": "TEXT",
                "source_packing_no": "INTEGER",
            })

            for table in ("loading_planning_carton_contents", "loading_planning_cartons",
                          "loading_planning_pallet_contents", "loading_planning_pallets"):
                conn.execute(f"DROP TABLE IF EXISTS {table}")

    @staticmethod
    def _add_columns(conn, table: str, columns: dict) -> None:
        """Guarded ADD COLUMN for a table that may not exist yet - a fresh
        install has no tables at all here, and schema.sql will create it in
        the right shape moments later."""
    ...
    def _pre_schema_drop_legacy(self) -> None:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v98's own columns are added in _pre_schema_loading_planning
            # instead of here, because one of them is indexed by schema.sql
            # and so has to exist before that script runs.

            # v100: packing_planning_items.job_in_id - a packing planning can
            # now load rows from a JOB IN's returned goods as well as from a
            # purchase order's production batches. NULL on every existing row,
            # which is a purchase-order row exactly as before. Not indexed, so
            # a plain guarded ALTER is enough.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(packing_planning_items)")}
            ...
            # every existing row, which prints as PLT exactly as before.
            for table in ("quotation_items", "proforma_invoice_items"):
                existing = {r["name"] for r in conn.execute(f"PRAGMA table_info({table})")}
                if existing and "packing_unit" not in existing:
                    conn.execute(f"ALTER TABLE {table} ADD COLUMN packing_unit TEXT")
