...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v31: export_invoices gains shipping_bill_date/stuffing_start_time/
            # stuffing_completion_time - see the changelog entry above.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoices)")}
            if existing:
                for col in ("shipping_bill_date", "stuffing_start_time", "stuffing_completion_time"):
                    if col not in existing:
                        conn.execute(f"ALTER TABLE export_invoices ADD COLUMN {col} TEXT")
