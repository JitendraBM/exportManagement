...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # gains excise_seal_no/plts/boxes (the reference's 11B table has
            # these three columns, we only had seal/vehicle/weight ones);
            ...
            if existing:
                ...
                for col in ("c_no", "c_date", "shipping_bill_no"):
                    if col not in existing:
                        conn.execute(f"ALTER TABLE export_invoices ADD COLUMN {col} TEXT")

            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoice_purchase_details)")}
            if existing:
                for col in ("supplier_invoice_qty", "supplier_taxable_amount",
                            "supplier_cgst_amount", "supplier_sgst_amount"):
                    if col not in existing:
                        conn.execute(f"ALTER TABLE export_invoice_purchase_details ADD COLUMN {col} TEXT")

            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoice_container_details)")}
            if existing:
                for col in ("excise_seal_no", "plts", "boxes"):
                    if col not in existing:
                        conn.execute(f"ALTER TABLE export_invoice_container_details ADD COLUMN {col} TEXT")
