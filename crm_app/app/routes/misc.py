...
The list types are CURRENCY (name of currency + currency symbol) and
NATURE OF CONTRACT (a name), which fills the delivery-terms field on every
document whatever that document calls it. Editing happens inline on the
single page rather than through separate form pages, since a list row is
only a field or two.
"""
...
def _fields(form) -> dict:
    return {"name": form.get("name", ""), "symbol": form.get("symbol", "")}
