...
def index():
    ...
    return render_template(
        ...
        countries=service.list_countries(g.user.company_id),
        units=service.list_units(g.user.company_id),
...
def add_currency():
    ...
    try:
        ...
        anchor = f"row-currency-{currency.id}"
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    return redirect(url_for("misc.index", _anchor=anchor))
...
def delete_currency(currency_id):
    if not verify_delete_password(g.user, request.form):
        ...
        return redirect(url_for("misc.index", _anchor="section-currency"))
    try:
...
@misc_bp.route("/api/currencies", methods=["POST"])
@admin_required
def api_quick_create_currency():
    """Lets an admin add a currency to the list straight from the Currency
    dropdown on the quotation / proforma-invoice / purchase-order / purchase-
    invoice forms (see the currency_select macro), so a missing currency
    doesn't cost a detour to Administration -> Miscellaneous and a lost
    half-typed document. Same shape as the port of loading quick-create;
    everything else about the list is still managed on the Miscellaneous
    page."""
    try:
        currency = current_app.container.misc_list_service.create_currency(g.user, _fields(request.form))
    except (ValidationError, PermissionDeniedError) as e:
        return jsonify({"error": str(e)}), 400
    return jsonify({"id": currency.id, "name": currency.name, "symbol": currency.symbol})
...
def delete_nature_of_contract(entry_id):
    if not verify_delete_password(g.user, request.form):
        ...
        return redirect(url_for("misc.index", _anchor="section-noc"))
    try:
...
def delete_port_of_loading(entry_id):
    if not verify_delete_password(g.user, request.form):
        ...
        return redirect(url_for("misc.index", _anchor="section-pol"))
    try:
...
def delete_container_type(entry_id):
    if not verify_delete_password(g.user, request.form):
        ...
        return redirect(url_for("misc.index", _anchor="section-container_type"))
    try:
...
@admin_required
def add_country():
    anchor = "section-country"
    try:
        entry = current_app.container.misc_list_service.create_country(g.user, _fields(request.form))
        flash(f"Country {entry.name} added.", "success")
        anchor = f"row-country-{entry.id}"
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    return redirect(url_for("misc.index", _anchor=anchor))


@misc_bp.route("/countries/<int:entry_id>/edit", methods=["POST"])
@admin_required
def edit_country(entry_id):
    anchor = "section-country"
    try:
        entry = current_app.container.misc_list_service.update_country(
            g.user, entry_id, _fields(request.form),
        )
        flash(f"Country {entry.name} updated.", "success")
        anchor = f"row-country-{entry.id}"
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    except NotFoundError:
...
@misc_bp.route("/countries/<int:entry_id>/delete", methods=["POST"])
@admin_required
def delete_country(entry_id):
    if not verify_delete_password(g.user, request.form):
        flash("Incorrect password. Country not deleted.", "error")
        return redirect(url_for("misc.index", _anchor="section-country"))
    try:
        entry = current_app.container.misc_list_service.delete_country(g.user, entry_id)
        flash(f"Country {entry.name} deleted.", "success")
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    except NotFoundError:
        abort(404)
...
@misc_bp.route("/api/countries", methods=["POST"])
@admin_required
def api_quick_create_country():
    """Lets an admin add a country to the list straight from the Country
    Name dropdown on the Buyer form (see the country_select macro), so a
    missing country doesn't cost a detour to Administration -> Miscellaneous
    and a lost half-typed buyer. Same shape as the port of loading /
    currency quick-creates; everything else about the list is still managed
    on the Miscellaneous page."""
    try:
        entry = current_app.container.misc_list_service.create_country(g.user, _fields(request.form))
    except (ValidationError, PermissionDeniedError) as e:
        return jsonify({"error": str(e)}), 400
    return jsonify({"id": entry.id, "name": entry.name})


@misc_bp.route("/units", methods=["POST"])
@admin_required
def add_unit():
    anchor = "section-unit"
    try:
        entry = current_app.container.misc_list_service.create_unit(g.user, _fields(request.form))
        flash(f"Unit {entry.name} added.", "success")
        anchor = f"row-unit-{entry.id}"
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    return redirect(url_for("misc.index", _anchor=anchor))


@misc_bp.route("/units/<int:entry_id>/edit", methods=["POST"])
@admin_required
def edit_unit(entry_id):
    anchor = "section-unit"
    try:
        entry = current_app.container.misc_list_service.update_unit(
            g.user, entry_id, _fields(request.form),
        )
        flash(f"Unit {entry.name} updated.", "success")
        anchor = f"row-unit-{entry.id}"
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    except NotFoundError:
...
@misc_bp.route("/units/<int:entry_id>/delete", methods=["POST"])
@admin_required
def delete_unit(entry_id):
    if not verify_delete_password(g.user, request.form):
        flash("Incorrect password. Unit not deleted.", "error")
        return redirect(url_for("misc.index", _anchor="section-unit"))
    try:
        entry = current_app.container.misc_list_service.delete_unit(g.user, entry_id)
        flash(f"Unit {entry.name} deleted.", "success")
    except (ValidationError, PermissionDeniedError) as e:
        flash(str(e), "error")
    except NotFoundError:
        abort(404)
